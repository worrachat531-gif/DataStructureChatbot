#!/usr/bin/env python3
"""
Document reader module for KMUTNB chatbot
"""
...
class DocumentReader:
    ...
    def get_document_summary(self) -> str:
        """Get a summary of the document content for API usage"""
        if not self.validate_file():
        ...
        try:
            ...
            # Read only the first few paragraphs for summary
            for i, paragraph in enumerate(doc.paragraphs):
                if i < 50:  # Limit to first 50 paragraphs
                    if paragraph.text.strip():
                        content.append(paragraph.text)
                else:
                    break
            
            # Read table headers only
            for i, table in enumerate(doc.tables, 1):
                if i <= 5:  # Limit to first 5 tables
                    content.append(f"\n--- TABLE {i} ---")
                    if table.rows:
                        # Add header row
                        header_data = [cell.text for cell in table.rows[0].cells]
                        content.append(" | ".join(header_data))
                    content.append(f"--- END TABLE {i} ---\n")
            
            summary = '\n'.join(content)
            
            # Add a note about the full document
            summary += "\n\n[หมายเหตุ: เอกสารนี้มีข้อมูลเพิ่มเติมเกี่ยวกับการรับสมัคร คณะ สาขา และหลักสูตรต่างๆ ของ KMUTNB]"
